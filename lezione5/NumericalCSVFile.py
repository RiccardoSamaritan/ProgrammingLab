class CSVFile:
    ...
    def __init__(self, name):
        ...
        if type(self.name) != str:
            raise Exception('Ho avuto un errore, il nome del file non è una stringa.')
    ...
    def get_data(self, start=None, end=None):
        ...
        if not self.can_read:
            ...
            # Controllo che i valori dell'intervallo siano degli interi
            if type(start) or type(end) != int:
                raise Exception ('Errore nei valori dell intervallo, devono essere interi: "{}", "{}"'.format(start, end))
            
            # Controllo che l'intervallo di lettura non sia maggiore della lunghezza del file
            if end > len(my_file.readlines()):
                raise Exception('Errore, intervallo maggiore della lunghezza del file da leggere')

            # Controllo che l'inizio dell'intervallo non sia maggiore della fine
            if type(start) > type(end):
                raise Exception ('Errore nei valori dell intervallo: "{}", "{}"'.format(start, end))
